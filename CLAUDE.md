...
- **The app icon has one source, `assets/icon/gawk.svg`.** The PNG set, the
  `.ico` and the `.res` beside it are generated by `go run ./tools/icon
  generate` and pixel-drift-checked in CI; never hand-edit them, and never
  add a second copy of the mark for a platform to consume (`docs/53`).
