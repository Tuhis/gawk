...
**Status**: designed 2026-09-15; **shipped 2026-09-16** (OA1–OA4);
**revised 2026-09-21** (OA5: the API page's Console). Chunks
**OA1–OA5** (`OA` = OpenAPI; two-letter prefix per the R21+ convention). `gawk-admin` only —
...
| D5 | **The SPA gets an "API" view at `#/api`, behind the normal login, rendering the served document with an embedded **Redoc**, lazily loaded.** *(Revised 2026-09-17; the first cut used `swagger-ui-dist` for its "Try it out" button.)* The bundle is a separate Vite chunk imported on first navigation, so the moderation views' load cost is unchanged. **No credential reaches it**: Redoc renders and does not execute, so there is no token to inject and no same-origin guard to get right. **Revised again 2026-09-21 (OA5): the page gains an in-house "Console" drawer** (`ui/src/views/ApiConsole.tsx`, `apiConsole.ts`) that sends one documented operation as the signed-in operator and shows the response. It is built from the served document — an operation picker grouped by tag, a field per path/query parameter (a `<select>` for enums), the documented request example pre-filled as an editable JSON body — and it sends through `AuthSession.authorizedFetch` with a **relative `api/v1/…` path** the module builds; there is no URL field. Anything but `GET` asks once before sending. "Copy as curl" writes the §9.8 shape with a `$TOKEN` placeholder, never the token. The drawer opens on the operation the Redoc hash names, follows sidebar clicks through Redoc's exported `history` service, and links back to the docs. Redoc itself is unchanged and still token-free. | The page is read far more often than it is used as a REPL, and it has to read well — owner decision 2026-09-17, overriding the first cut's reasoning that executing a call mattered more than the layout. `curl` with a client-credentials token (self-hosting §9.8) is what replaced "Try it out", and it is what a bot author ends up writing anyway. The swap also removed the one reason a third-party bundle in the security-critical SPA was ever handed an access token. Embedded, never a CDN: the portal's CSP is `default-src 'self'` and the no-external-assets test enforces it (docs/42 §4.8). The view is behind login because it is a page of the admin portal, not because the document is secret (D2). **OA5's rationale** (owner question 2026-09-21: *can the API page try calls with the current user's token?*): the part of "Try it out" worth having is sending a call with the token already in hand; the part that cost was a third-party bundle holding it. An in-house drawer keeps the first and drops the second: the token stays where `client.ts` keeps it, the path cannot leave the origin because the module never builds an absolute one (`apiConsole.test.ts` pins this), the CSP is untouched, and the readability decision stands because the drawer is a tool picked up beside the reference, closed on every visit, not a mode of the page. Swagger UI beside Redoc — two large bundles and the token problem back — is the option not taken (§2). |
...
- **Bringing Swagger UI back beside Redoc for a "Try it" tab** (considered
  2026-09-21 when the owner asked for in-browser calls) — two ~1 MB bundles
  on one page, and the operator's access token handed to a third-party
  bundle again, with the same-origin guard that implies. The in-house
  Console (D5, OA5) is the answer: a few hundred lines that send through
  the session like every view does.
- **Injecting a "Try it" button into Redoc's operation panels** — Redoc's
  DOM is generated and its class names are styled-components hashes that
  change on every upgrade (`ApiView.css` already avoids them). The drawer
  links to and from Redoc through the hash it writes instead.
...
| SPA | `ui/src/router/router.ts:42` `VIEWS`; `ui/src/views/*` | `api` added to `VIEWS`; **new** `views/ApiView.tsx`, lazily imported by `App.tsx`, rendering `<RedocStandalone>`; `views/apiDocs.ts` holds the options and the relative document URL; nav link. No token holder is involved. **OA5 (2026-09-21)**: `views/ApiConsole.tsx` + `ApiConsole.module.css` (the drawer) and `views/apiConsole.ts` (the pure model: operations from the document, relative request paths, the curl line, the Redoc hash) join the same lazy chunk; `ApiView.tsx` gains the "Console" toggle and the `withConsole` wrapper class that narrows Redoc while the drawer is open. The drawer sends through `useSession().authorizedFetch` — the token holder the views already use. |
...
| **OA5** (2026-09-21) | The API page's **Console** drawer (D5 revised): `apiConsole.ts`, `ApiConsole.tsx`, the toggle in `ApiView.tsx`; docs (this document, ROADMAP, README, self-hosting §9.8) | `apiConsole.test.ts`: every operation of a fixture document is listed with method, tag, role, parameters (`$ref` followed, path ones required, enums and defaults read) and pre-filled example; `buildRequestPath` yields `api/v1/…` **with no leading slash**, encodes path values, throws on an empty **required** value — path or query alike (review of PR #337) — and sends only the optional query values that were filled; `curlFor` carries `$TOKEN` and the served `servers[0].url`. `ApiConsole.test.tsx`: the request leaves through the **session stub** (`authorizedFetch`, relative path, method, compact JSON body, `Content-Type`); a `GET` sends on one click, anything else shows the caution and sends only on the second; invalid JSON and an empty path parameter are refused before anything leaves; the token appears nowhere in the DOM or the curl line; 204 renders as "(no body)", a 4xx in the failure colour with the error envelope; sensitive responses carry the document's reason; the drawer opens on the Redoc hash, follows a `history` emit, resets on operation change, closes on Escape and the button. `ApiView.test.tsx`: the drawer is closed on mount, opens from a **floating** toggle (fixed, visible however far down the reference the reader is; gone while the drawer is open, back on close — owner tweak 2026-09-21), adds `withConsole` to Redoc's wrapper, and Redoc is still handed no token. `npm run lint`/`build` clean, the console in the `ApiView.js` chunk only; `internal/portal` no-external-assets green. **Manual on the docs/41 dev stack** (`--profile sim`): log in, open API → Console, send `GET /api/v1/me` and `GET /api/v1/broadcasts`, walk a mutation to its confirm step and cancel, click a sidebar operation and see the console follow, check the light scheme and a phone width. |
...
page executes the same call from its Console (OA5); and in the repository, adding a route
...
- **The API page runs a third-party bundle inside the security-critical
  ...
  for one.
- **The Console (OA5) sends with the operator's token, and sees none of
  it.** The request goes through `AuthSession.authorizedFetch`, the one
  place in the SPA that attaches a bearer header — the same seam every
  view uses — with a path `apiConsole.ts` builds from the served document:
  relative, `api/v1/…`, so there is no origin in it to get wrong and no
  same-origin guard to write. There is no URL field, so an operator cannot
  be talked into pointing their token at another host from this page. The
  curl line carries `$TOKEN`, never the token, because it is meant to be
  pasted into a chat or a ticket. Mutations confirm once; a 403 flips the
  session to its forbidden page exactly as a view's would, because the
  API has one role and that 403 is about the session, not the call. The
  console is first-party code in the Redoc chunk, not a new dependency.
