...
| D7 | **Outbound webhooks are documented under OpenAPI 3.1's top-level `webhooks`**, one entry per webhook-eligible type (`store.WebhookEventTypes()`, D3 (d)), sharing one `ModerationEvent` schema, with the four `X-Gawk-*` headers and the `Sign()` construction spelled out in the description. | 3.1 is chosen over 3.0 for this one feature: a receiver author gets the payload, the headers and the replay window from the same document as the API, instead of from prose in §9.5 alone. D3 (d) keeps the type list honest. |
...
| Event types | `internal/store/store.go:126-138`; `store.AllEventTypes()` | A second helper, `store.WebhookEventTypes()`, the webhook-eligible subset (today equal to `AllEventTypes()`; R49 D8 is where the two diverge). D3 (d) reads this one, and the dispatcher's filter (R49) validates against the same helper. |
