...
| D3 | **A Go test is the drift gate, both directions.** `Routes()` builds from a declared table (`method`, `pattern`, `roles`, handler) instead of inline `mux.Handle` calls; the test loads `openapi.yaml`, and asserts: (a) every table entry has a matching `paths.<pattern>.<method>`, with `{id}` placeholders equal; (b) every operation in the document has a table entry — no documented-but-missing route; (c) every `Code*` constant appears in the error schema's `code` enum and vice versa; (d) every `store.Event*` type appears in the webhook schema's `type` enum and vice versa; (e) each operation's `x-gawk-roles` matches the table's roles; (f) every response example in the document decodes into the handler's Go response type with `DisallowUnknownFields`, and every request example into its request type. | The R2 lesson in API form: a contract nobody is forced to update is a contract that lies. (f) is the cheap, dependency-free half of schema validation: an example that names a field the struct lost fails, and a struct that gains a field the example lacks is caught by a second pass that marshals a fully-populated fixture and asserts every key appears in the schema's `properties`. Full JSON-Schema validation would need `kin-openapi`; it is listed in §2 Rejected with the reason. |
| D4 | **The document is also linted as OpenAPI in CI**, by `@redocly/cli lint` run from the `admin-ui` job (a pinned devDependency in `ui/package.json`, so the lockfile fixes the version). Ruleset: `recommended`, plus `operation-operationId` and `no-unused-components` as errors. | D3 checks the document against the code; this checks the document against the standard. A malformed `$ref` or a missing `operationId` would otherwise surface only when someone feeds the file to a generator. The UI job already has node and runs lint; no new job, no new runner image. |
...
| D7 | **Outbound webhooks are documented under OpenAPI 3.1's top-level `webhooks`**, one entry per `store.Event*` type, sharing one `ModerationEvent` schema, with the four `X-Gawk-*` headers and the `Sign()` construction spelled out in the description. | 3.1 is chosen over 3.0 for this one feature: a receiver author gets the payload, the headers and the replay window from the same document as the API, instead of from prose in §9.5 alone. D3 (d) keeps the type list honest. |
| D8 | **`info.version` is the `gawk-admin` release version**, maintained by release-please's `extra-files` (`type: yaml`, `jsonpath: $.info.version`) exactly as `version.go` is for the broadcasters. The served document carries the same value; `x-gawk-build` carries the ldflags build string. | A consumer reading the repository file and one fetching it from a deployment must agree on which version they are looking at. If the yaml updater misbehaves in the release PR (unverified today — this is the one owner-confirmed risk in OA2), the fallback is serve-time injection from the build version with the file pinned at `0.0.0`. |
| D9 | **What the document promises, in its own words.** `info.description` states: `/api/v1` is additive within v1 — fields and operations are added, never removed or retyped; a removal is a `/api/v2`; the error `code` enum and the event `type` enum only grow; examples carry fictional identifiers. Every response that can carry a raw broadcast ID, a room code or an IP is marked `x-gawk-sensitive: true` with a one-line reason, and the webhook schema is marked `x-gawk-sensitive: false` with docs/42 D8 cited. | The consumer needs to know what may change under them. Additive-only is what §4.10's `enforcement` and `roomKey` fields already practised without saying so; saying so makes it a rule D3 can hold the next change to. The sensitivity marks make the D8 relaxation visible where the consumer reads, not only where the maintainer does. |
...
|---|---|---|
| Route registration | `internal/api/api.go:213-262` inline `mux.Handle` calls | A `routeTable` slice `{method, pattern, roles, handler}`; `Routes()` ranges over it. Behaviour identical; the table is what D3 enumerates. |
| Error codes | `api.go:60-71`, `rooms.go:42-50` | Unchanged; D3 (c) reads them via a small `allErrorCodes()` helper. |
| Event types | `internal/store/store.go:126-138` | Unchanged; D3 (d) reads them via `store.AllEventTypes()`. |
| The document | — | **New** `gawk-admin/openapi.yaml` (3.1): every route in §4.7 of docs/42 as shipped (§11.1 deviations included), the rooms routes, `webhooks`, the schemas, examples, `x-gawk-roles`, `x-gawk-sensitive`. |
| Serving | `cmd/gawk-admin/main.go:205-224` mux; `internal/portal` embed pattern | **New** `internal/openapi` package: `//go:embed openapi.yaml`, YAML→JSON once, `servers` rewrite, `Handler()`; wired as `GET /api/v1/openapi.json` **before** the `/api/v1/` catch-all so it is not swallowed by the 404 handler and not wrapped by `protect()`. |
...
| Release automation | `release-please-config.json` `extra-files` for the broadcasters' `version.go` | The same for `gawk-admin/openapi.yaml` `$.info.version` (D8). |
...
- **Changing the API**: edit the handler and the document in the same
  ...
  patch (D9).
- **Version mismatch between file and served copy** points at the D8
  updater; the served `x-gawk-build` says which binary answered.
