...
**Status**: designed 2026-09-16; **shipped 2026-09-17 (EC1–EC4)**; **revised
2026-09-22 — D9 makes every event name its broadcast or room in cleartext, on
both channels** (§8). Chunks
...
| D9 | **Events name things in cleartext** *(2026-09-22, revising D1 and D4)*. The CloudEvents `subject` is the **raw broadcast ID or room code**, on both channels, and `data` carries the raw identifier beside the HMAC'd key on both channels too: **the webhook projection strips nothing**. The `x-gawk-sensitive` marks stay in the schemas and change meaning — they warn a consumer that the property is a join capability, and the AsyncAPI document says so in those words ("point a webhook at a channel whose readers you would let in"). Three things do **not** move: no publisher IP is in any event, so none can be delivered; `portalUrl` is still built from the HMAC'd key, because that is what the portal's routes take; and the **NATS subject's key token stays HMAC'd** (docs/51 D2), because that token is what shows up in bus monitoring and in an operator's `nats sub '>'`, which is a listing of subjects rather than a delivery of events. **Every room event also carries `displayCode`** — the code with its casing, which `roomCode` normalises away — where before only `room.opened` did; it is an additive property under D6 (b), and `gawk-admin`'s room rows already recorded it. **This is an exception to D6 (c), taken knowingly.** `subject` changes meaning under the same `type` and `dataschema`, which D6 (c) would route through a versioned type kept alongside the old one for 90 days. It is taken without that because neither channel has a consumer outside the reference deployment yet — the posture D5 took when it replaced the signature scheme outright. It is not a precedent: from here on, a change of meaning to an envelope attribute is incompatible in exactly the way a `data` change is, and takes the D6 (c) route. **Upgrade order**: with the bus on, `gawk-admin` goes before `gawk-server` for this release (self-hosting §12.3). An older portal reads the key from `subject` and would store the raw identifier as the row's key; a newer portal reads it from `data`, which every relay has sent. Gates: `TestSubjectIsTheCleartextIdentity` over every vector in `gawk-server/events`, and `TestEveryVectorProjectsWhole` (which fails a vector that leaves a sensitive property unpopulated) plus `TestNoIPOrStraySecretInAnyDelivery` in `gawk-admin` — whose fixture summary is the real summariser's, so the identity check covers the sentence a receiver actually gets. | A notification whose subject is `35303d0cbae0` cannot be acted on without first resolving a digest, so it gets ignored — which makes the whole pipe worth less than the exposure it was avoiding. The exposure is smaller than D8 assumed: a receiver is not "third-party push infrastructure" in general, it is one channel the operator chose and configured, and the same operator is told (self-hosting §9.5) to choose one they would be comfortable having read. The identifiers were already in the bus tier and in the portal; the honest boundary is the operator's choice of receiver, not a projection that hid the one field the notification is about. The HMAC'd key stays because the portal is keyed by it and a delivery that dropped it would break every deep link. |
...
- **What leaves the deployment** *(rewritten 2026-09-22 for D9)*: a webhook
  body carries the raw broadcast ID or room code — in `subject` and in the
  property the schema marks `x-gawk-sensitive` — and **never** a publisher
  IP, an operator's note or a room's attach secret. A delivery is therefore
  a join capability for exactly the broadcast or room it is about, and the
  boundary is the operator's choice of receiver (self-hosting §9.5). What
  the marks buy is that the choice is informed: they are in the document a
  receiver author reads, and the D7 fixture test still proves nothing else
  slips in beside them.
...
- **The key rule lives in `config.SigningKey`** — optional `whsec_` prefix,
  then always base64-decode, exactly as the reference libraries do (revised
  2026-09-18, PR #327 review; the first cut used a non-prefixed secret
  verbatim, which no library does). Both webhook sources reach it:
  chart-defined secrets are checked at parse time and refuse to start, a
  portal-created secret that does not decode is a `400`, and the dispatcher
  derives the key at every send. Generate one with `openssl rand -base64 32`. A test send's `id` is a fresh
...
## 8. Revision: cleartext identities (2026-09-22)

D9 replaced two of the original decisions, after the first real receiver was
pointed at the bus: every notification arrived identified only by a digest
(`subject: "35303d0cbae0"`), which is unactionable without first resolving it
in the portal — so the pipe was worth less than the exposure it was avoiding.

What changed:

- `subject` is the raw broadcast ID or room code, on the bus and in every
  webhook delivery (D1's attribute table, and `TestSubjectIsTheCleartextIdentity`
  over every golden vector).
- The webhook projection strips nothing. `project` in `gawk-admin` now only
  adds `summary` and `portalUrl`; the golden delivery bytes and every pinned
  Standard Webhooks signature vector were recut over the new body.
- `x-gawk-sensitive` stays in the schemas with a new meaning: a warning that
  the property is a join capability, stated in the AsyncAPI document in the
  words a receiver author reads.
- **`displayCode` is on every room event**, not just `room.opened`. `roomCode`
  is the normalised code — lower case, the CR name and the registry key — and
  a notification that said `tuhistestlab` when the operator named the room
  `TuhisTestLab` would make them translate it themselves. The golden vectors
  now show the two differing (`r7k3mx` / `R7K3MX`), which is what a producer
  actually emits; they used to show the display form under both names.
- `gawk-admin` reads the HMAC'd key from `data` (`broadcastKey` / `roomKey`)
  rather than from `subject`, because the portal's rows, links and filters
  are keyed by it.

What did not change: no event carries a publisher IP; `portalUrl` is still
built from the HMAC'd key and still grants nothing without logging in; and
the **NATS subject's key token stays HMAC'd** (docs/51 D2) — a listing of bus
subjects in monitoring is not a delivery of events.

**Summaries follow the same rule.** The one sentence a push bridge renders
names what the event is about the way the event's own properties do:

- A kill names the raw ID — "broadcast ABC123 was terminated by …" — where
  it used to name the HMAC'd key.
- A ban names the broadcast it bans ("a ban on broadcast ABC123 was created
  by …"), or, for an IP ban, the broadcast it was taken from ("a ban on the
  publisher IP of broadcast ABC123 …"). **It never names the address.** With
  no broadcast on record the sentence falls back to "a broadcast ban" / "a
  publisher IP ban", as before.
- A room sentence names the room by its display code and its kind ("static
  room TuhisTestLab was created by …", "dynamic room R7K3MX ended"), where it
  used to name the kind alone. Activity summaries prefer the display code
  over the normalised subject for the same reason.

A ban's three events (created, expired, removed) now all take the broadcast
from one rule — the ban's source broadcast, or its target when the ban is on
a broadcast ID — so they name the same broadcast. The create and removal
paths in `internal/api` used the source alone and named nothing for a ban
typed in by ID; the expiry path and the reconciler already used the full rule.
The rule is one method, `store.Ban.BroadcastID`, rather than the two copies
it used to be, so the two packages cannot drift apart on it. The HMAC'd key is
still looked up only for a ban taken from a live broadcast: it feeds the portal
link alone, and a ban typed in by ID does not wait on a fleet snapshot.

The golden delivery and every pinned signature vector were recut a second
time for the kill summary, again computed outside the package.

**Found in review (PR #348), fixed with a regression test each:**

- Ingest took the row's key from `broadcastKey` first, so `room.attached`,
  `room.detached` and `room.attachment_updated` — which carry both keys —
  were stored under the broadcast's key rather than the room's, where the old
  `subject` had put them. The room's key wins now.
- The fallback summary, used for a row stored without one, passed no target
  type, so an IP ban taken from a broadcast was announced as a ban on the
  broadcast. It now reads the target's type from the row through
  `store.Event.TargetType`, a closed vocabulary that never reads the value.
- The poison test's fixture summary named nothing, so it never exercised a
  summary carrying an identity. It now uses the real summariser, and a
  summary may name only the event's own identity.
