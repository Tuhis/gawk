...
| [`49`](49-admin-openapi.md) | R48 — OpenAPI contract for the `gawk-admin` API: served, drift-checked, with an embedded Redoc page and an in-browser Console (OA5) |
...
| [`56`](56-admin-mcp.md) | R54 — MCP server for the `gawk-admin` API: Claude Code acts as the signed-in operator via the MCP OAuth flow against the deployment's IdP; tools generated from the OpenAPI contract, mutations opt-in |
