...
- **Live means the page is running (2026-09-06).** An attachment's
  `live` flag came from "the publisher session is up", so a broadcaster
  tab left in the background — page frozen, session answering keepalives —
  showed a black tile marked live for hours, and a second tab from the
  same person produced a second tile with the *same label*. Two relay-side
  fixes, neither a wire change: the hub's `BroadcastState` now reports a
  publisher from which no datagram at all has arrived for
  `-publisher-stall-timeout` as not live (docs/06 revision 2026-09-06: away
  at 90 s of silence — TimeSync and ClockMapping count, so a paused game
  or a static screen is not away; ended at the broadcast grace only with
  the opt-in `-publisher-stall-ends`), which the room registry's refresh
  turns into the existing away state; and the registry de-duplicates
  attachment **labels** the way it de-duplicates nicknames (D10) — `pc`,
  `pc (2)`, `pc (3)` — at attach and at a relabelling re-attach
  ...
  natives included, within the same refresh interval fleet-wide — on the
  origin pod from its hub, on every other pod from the origin Lease, which
  the origin stamps (`gawk/stalled-since`) at the onset and clears on
  recovery, read by `Coordinator.Lookup` for an attachment nobody there
...
- **End room asks first.** It is a quiet destructive line ("End room…")
  ...
  drops the confirm.
- **The creator is not offered "Start streaming here".** The creator flag
  arrives only with the creator token, and a tab holding it with no
  broadcast of its own is a native app's "Open room view" (the web
  broadcaster's own view already has its stream attached) — that person
  already streams from the app, so a second, browser stream is noise.
  Neither the empty-room card nor the panel shows it; a creator whose
  native stream stopped restarts it from the app.
- **The creator's role is visible.** A "Creator" chip in the room accent
  sits beside the code chip (and beside the broadcaster's room pill); a
  click opens what the role can do, worded to what the token actually
  grants (D8): remove any *stream* — it keeps running on its own code and
  may be attached again — and end the room for everyone. It says plainly
  that people who are only watching cannot be removed: there is no
  command for it. "Creator", not "owner", because the removal and ending
  toasts already say "the room's creator". The open help counts as an
  overlay, so the header does not fade while it is read.
- **The phone header fits.** Below 720px the watching count leaves the
  header (the People panel has it) and below 375px the streaming count
  too; the row of code, counts and two icons already overlapped below
  ~400px before the chip. The help spans the header there.
...
the same confirm; a creator with no stream in the tab sees End room but
no "Start streaming here"; the creator (only) has the Creator chip, whose
help names stream removal, ending the room and the no-people limit, and
closes on Escape or a second click. `BroadcasterScreen.room.test.tsx` —
the room pill carries the copy button and a minting broadcaster gets the
Creator chip. The phone header was measured at 360/390/430px on the dev
stack.
