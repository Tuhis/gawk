...
the path-inserted location for its resource —
`/.well-known/oauth-protected-resource/mcp` for a resource at `/mcp`
(RFC 9728 §3.1) — naming its authorization server(s),
...
1. **The spec path.** `/mcp` returns the challenge; the metadata document
   names `-oidc-issuer` as the AS and **the `/mcp` URL itself** as the
   `resource` — RFC 9728 §3.3 requires `resource` to equal the identifier
   the metadata URL was derived from, and a client discards a document
   where they differ. `-oidc-audience` stays what the verifier checks in
   `aud`; it is not the `resource`. Telemetry has no external-URL knob and
   must not grow one (D5: the same pod is reached at the Ingress host *and*
   at `localhost:8081` through a port-forward), so the resource URL is
   **derived from the request** — scheme, `Host`, then `/mcp`. The scheme
   comes from `X-Forwarded-Proto` when present (first value), else from
   whether the connection itself is TLS. The read Ingress terminates TLS
   and proxies plain HTTP to `read.port`, so the pod never sees `https`
   itself. Without the header the reference deployment would advertise
   `http://…/mcp` to a client that called `https://…/mcp`, and the client
   would have to discard it. Trusting the header is safe for the same
   reason the derivation is: the document's only authority-bearing field is the
   fixed issuer, and a client checks the echoed `resource` against the URL
   it called. No copy is served at the root
   `/.well-known/oauth-protected-resource`, which under §3.1 would describe
   ...
   **V-4 in §10**, not an assumption. *(2026-09-22: [docs/56](56-admin-mcp.md)
   ...
   metadata helpers are **built here, in TO4**, as generic functions in
   `oidcauth` taking the issuer and a resource-URL function; R54 MC3
   reuses them with `-external-url` + `/mcp`.)*
...
| **TO4** | MCP: `401` challenge with `resource_metadata`, `/.well-known/oauth-protected-resource/mcp` (path-inserted, no root copy; D6), `Origin` validation on `/mcp` as the MCP spec requires (any `Origin` header → 403; D6); the challenge and metadata handler as generic helpers in `gawk-server/oidcauth` (issuer + a resource-URL function) for R54 to reuse | Go tests for the challenge and the metadata document (`resource` equals the `/mcp` URL derived from the request — asserted for an Ingress `Host` with `X-Forwarded-Proto: https` (→ `https://…/mcp`), the same `Host` without the header (→ `http://`), and `localhost:8081` — `authorization_servers` is exactly the issuer, `bearer_methods_supported: ["header"]`); the root `/.well-known/oauth-protected-resource` is not served; the existing MCP-vs-HTTP byte-identity test still passes with a bearer supplied. |
