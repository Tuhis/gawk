...
1. **The spec path.** `/mcp` returns the challenge; the metadata document
   ...
   **derived from the request's own origin** — scheme, `Host`, then
   `/mcp` — as the SPA's redirect URI is derived from the page's origin.
   That is safe here: the document's only authority-bearing field is the
   ...
   the bare origin *(revised 2026-09-23 with [docs/56](56-admin-mcp.md)
   D5)*.
...
| **TO4** | MCP: `401` challenge with `resource_metadata`, `/.well-known/oauth-protected-resource/mcp` (path-inserted, no root copy; D6), `Origin` validation on `/mcp` as the MCP spec requires; the challenge and metadata handler as generic helpers in `gawk-server/oidcauth` (issuer + a resource-URL function) for R54 to reuse | Go tests for the challenge and the metadata document (`resource` equals the `/mcp` URL derived from the request — asserted for both an Ingress `Host` and `localhost:8081` — `authorization_servers` is exactly the issuer, `bearer_methods_supported: ["header"]`); the root `/.well-known/oauth-protected-resource` is not served; the existing MCP-vs-HTTP byte-identity test still passes with a bearer supplied. |
