...
| DVR viewer (R21) freezes despite ring-backed delivery | Overlay Delivery mode says `ring-backed (buffer N ms)`; relay `gawk_broadcast_dvr_resyncs_total` climbing for that broadcast (per-sub `dvrResyncs` in `/statusz`) | **Stalls are outliving the ring** — the cursor fell off the tail, which is the mode's one frame loss (docs/26 Decision 4). Raise `-dvr-window`, or accept it: the buffer must strictly exceed the stall it covers, and covering `S` with buffer `B` needs `B/(B−S)`× burst bandwidth. If `dvrResyncs` is flat while `dvrLagMs` is large and `dvrGopSeq` is climbing, nothing is wrong — that is a viewer riding out a bad link exactly as designed |
| Relay memory climbing with resilient viewers (R21) | `gawk_broadcast_dvr_ring_bytes` near `-dvr-max-bytes`, `gawk_broadcast_dvr_subscribers` > 0 | **The ring is doing its job and costing what it costs** — one window per *broadcast*, not per viewer, so more viewers do not add memory. 3 s of a 50 Mbps broadcaster is ~18 MB; the byte cap binds before the window does and evicts oldest-first, which shows up as `dvr_resyncs_total` rather than as an OOM |
| Live-edge viewer (R29) still losing frames with parity on | Overlay Parity rows: `Parity chunks` climbing but `Parity recovered` low against `Dropped (incomplete)`; relay `gawk_broadcast_parity_datagrams_total` rising for that broadcast | **How MUCH parity recovers is the discriminator.** Repairing plenty and still behind = the loss exceeds what this level covers → raise `-parity-default`. Repairing almost nothing = the loss is **bursty**, and no per-frame code covers that at any level (docs/34 §9: even k=4 leaves ~37 % of GOPs damaged under a burst model) → route that viewer to Resilient mode, whose QUIC retransmission does not care how loss is distributed. `diagnose()`'s `parity-ineffective` rule splits these two automatically |
| Live-edge viewer (R29) asked for parity and is not getting it | Overlay shows `Parity chunks` at 0 while the fleet's `-parity-default` > 0 | **The producer is not emitting.** The relay advertises the level via RelayCapabilities and a producer that never sees it stays silent — so check the broadcaster's own `parityLevel` stat, and that it is not an older build. A carrier-mode viewer reading 0 is correct and not a fault: the relay serves those none on purpose |
| Fleet egress up ~22 % after enabling R29 | `gawk_relay_egress_parity_bytes_total` against `gawk_relay_egress_bytes_total{kind="delta"}` | **That is the feature's price, and it is measurable rather than modelled.** Parity bytes are a SLICE of the delta total, never a sibling — adding the two double-counts. If the ratio is unaffordable, `-parity-default` is the one value that turns it down fleet-wide (0 makes the wire byte-identical to pre-R29); viewers can already opt down individually |
| Resilient viewer (R21) asked for a ring and did not get one | Overlay Delivery mode says `reliable (resilient)` rather than `ring-backed`, with no `dvrBufferMs` | **The relay declined or predates R21** — the join-time DeliveryAck is the only way to tell, because a replayed GOP is byte-identical to a live one. Check the relay's `-dvr-window` and that the viewer asked for at least `MinDVRBufferMs`; below that the relay downgrades on purpose, since every replayed record would arrive past due |
| Live-edge viewer (R30) loses chunks of large frames while small frames arrive clean | Overlay `Stripe detector` row: large-frame loss ≥ ~1 % with small-frame loss ~0; `Striping` row shows whether legs are active; relay `gawk_broadcast_stripe_legs` / `striped_primaries` | **A per-connection receive buffer is overflowing on bursts, not a lossy network** (docs/34 finding 4 — a ~8-packet threshold, head-of-burst, parity spared). Striping (docs/35) is the designed answer: if the row reads `auto — off` with the detector firing, check the relay advertises `CapStripedDelivery` (`-striped-delivery`), the subscriber caps have room for legs, and the viewer's Striping menu setting; if legs are ACTIVE and the loss persists, the split is not buying headroom on that path — route the viewer to Resilient mode, never add legs. `diagnose()`'s `burst-threshold-loss` rule splits these two automatically |
