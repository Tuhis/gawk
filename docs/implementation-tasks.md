...
For details on E–G, see [06-multi-broadcaster.md](06-multi-broadcaster.md).
...
| D4 CI/release (added) | ✅ done | release-please monorepo (combined release PR — separate PRs conflicted on the shared manifest; tags per-component `gawk-server-vX.Y.Z`), `ci.yml` (go/app/helm/docker), `release-please.yml` with chained GHCR publish. **First cycle completed 2026-07-12: v0.5.0 both, 4 GHCR packages verified**. Milestone D close-out 2026-07-12: homelab install live with automated deploy-on-release (cluster-side; CI stays publish-only), manual browser verify passed end-to-end — see `05-resilience-deploy.md` |
