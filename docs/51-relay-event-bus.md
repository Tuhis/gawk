...
- **The bus is internal-tier infrastructure carrying raw broadcast IDs
  ...
  `Room` CRs. NATS subjects carry HMAC'd keys only, which is why the key
  token stayed HMAC'd when the event's own `subject` went cleartext
  (docs/52 D9): `nats sub '>'` and every monitoring view list subjects. It
  must never be routed publicly, and the docs say so where the recipe is.
