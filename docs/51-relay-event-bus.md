...
still works and its activity webhooks simply never fire.
...
|---|---|---|
| Relay publisher | — | **New** `gawk-server/internal/eventbus`: `Event` types, `Publisher` (connect with reconnect, `PublishAsync`, ack tracking, drop counters), `Nop` when off; the `nats.go` import lives here only (D8). |
...
| Store + migration | `migrations/0001_initial_schema.up.sql`; `moderation_events` | `0002_activity_events.up.sql`: `category text NOT NULL DEFAULT 'moderation'`, `source text NULL UNIQUE`; `store.AllEventTypes()` grows the activity types; `SummarizeActivity`; `ListEvents` category filter; `PruneActivityEvents`. Expand-only; the previous release ignores both columns. |
...
| OpenAPI (R48) | `gawk-admin/openapi.yaml` | The `category` query parameter on `/events`, the `bus` object on relays, the activity event types in the enum. R48's drift test forces it. |
...
shows only HMAC'd keys in subjects, killing the `gawk-admin` leader loses
