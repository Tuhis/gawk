...
- **Since v0.12.0, `webtransport.Server.Config` must be set or WebKit refuses
  ...
  makes mandatory alongside it, and Safari (macOS and every iOS browser)
  declines before the extended CONNECT, so the relay logs nothing at all.
  `Server.Config` is the only door: keys written into
  `H3.AdditionalSettings` are wiped by `Server.init`. The relay sets the trio
