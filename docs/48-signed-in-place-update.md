...
| D8 | **Terms and INSTALL.md.** The R45 sentence in the R23 terms (docs/47 D12) already discloses the manifest fetch; this milestone adds the download to it: "…and, at the user's request, download the newer version from GitHub". INSTALL.md's SmartScreen section is revisited per SU3's outcome. | Same recommendation as docs/47 D12 on `termsVersion`: no bump unless the owner decides otherwise. |
...
| Docs | docs/38 D17; both INSTALL.md; `docs/self-hosting.md` or §6 here; `docs/gotchas.md` | SU5. |
...
| **SU5** | Docs (D8): INSTALL.md install-flow paragraphs, the terms sentence, key-rotation runbook (§6), gotchas, the ROADMAP row and entry | Review; the rotation runbook has been walked once on a throwaway key pair. |
