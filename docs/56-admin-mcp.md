...
| OD8 | Landing | **Two PRs**: MC1 alone (a refactor with a zero-change criterion), then MC2–MC5. **R53 lands first**; R54 is not reordered ahead of it. |
...
| CLAUDE.md | Every new knob is plumbed through flags, `GAWK_ADMIN_*` envs and Helm values. Raw broadcast IDs and publisher IPs never reach a webhook payload. |
...
issuer and a resource URL — and live in `gawk-server/oidcauth` beside the
verifier, where R53 TO4 needs the same two things for telemetry's `/mcp`.
Whichever of R53 and R54 lands second reuses them. They are not written
twice.
...
- **Broadcast IDs and room codes** are what `kill_broadcast` and the room
  ...
  joinable, but short-lived and already visible to anyone watching; the
  operator who connects an agent is choosing to let it act on them. **Kept.**
...
**Rejected — HMAC'd stand-ins** for redacted IPs (the webhook `key`
treatment). They would let the model notice "same publisher as before",
...
- **Webhooks**: `actorClient` is a client ID, not personal data and not a
  broadcast ID, so it is webhook-safe and is added to the webhook-safe key
  list. The D8 prohibition (never raw IDs or IPs) is unaffected.
...
this item adds (`Origin` validation, the `MCP-Protocol-Version` header,
`405` on `GET`) would otherwise be implemented in one copy and forgotten in
the other.
...
- An `Origin` header, when present, must equal the configured origin set
  (the service's own external origin); otherwise `403`. Native clients such
  as Claude Code send none; a browser page on another origin is refused.
  This is the spec's DNS-rebinding requirement, and it matters more here
  than for telemetry because this endpoint is internet-reachable.
...
Nothing in the relay imports `mcphttp`; the relay's containment test gains
...
Revocation is the portal's: remove `operator`, or end the client's session
in Keycloak, and the agent loses access at its next refresh — within one
...
**Depends on**: R53 TO1 (`gawk-server/oidcauth` exists; MC3 puts the
challenge and metadata helpers in it). R48 (the contract). Not on R49: its
...
**Two PRs** (OD8): MC1 first, titled `refactor(telemetry): …` with a
`gawk-server`-touching commit, reviewed against its zero-change criterion
...
| The MC1 refactor changes telemetry's MCP behaviour invisibly | G10: its tests pass unedited; MC1 lands alone. |
...
| # | Check | Result |
|---|---|---|
| V-1 | Claude Code, given only the MCP URL, follows the `401` to the metadata document and discovers Keycloak's authorization and token endpoints (record which well-known document it fetched) | |
...
| V-6 | Remove `operator` from the user in Keycloak: the next tool call after the access token expires fails, and Claude Code shows the server needing authentication | |
