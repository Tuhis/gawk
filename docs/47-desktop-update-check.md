...
| Docs | `gawk-broadcast/INSTALL.md:245-255`, `gawk-broadcast-windows/INSTALL.md:61-67`, both READMEs' diagnostics sections; the R23 terms `BundledTerms.tsx` §7 | One paragraph each naming the check, the destination (GitHub), what it carries (nothing), and the opt-out. The terms gain one sentence (D12). |
...
| **AU5** | Docs: both INSTALL.md and READMEs, the terms sentence (D12), `docs/gotchas.md`, this document's status, the ROADMAP row and entry | Review; `TermsPage.test.tsx` / `sanitize.test.ts` still pass; INSTALL.md's outbound-connections paragraph names three destinations (relay, telemetry, GitHub) and the opt-out for each. |
