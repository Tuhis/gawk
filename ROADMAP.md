...
| R34 | [Native Windows broadcaster](#r34--native-windows-broadcaster) | 🔧 designed 2026-07-30 (Rust + Media Foundation + Slint; owner decisions OD1–OD12); **WB0–WB2 implemented same day** (Cargo workspace + path-filtered CI + release-please component; wire mirror with golden vectors incl. Go-generated GF(256) parity pins; engine core — send policy, resume supervisor, timesync, dispatch — seam-tested; wtransport adapter with 5 vendored interop patches, docs/38 §11; 4-test integration suite green against the real relay: publish, 401/403/404 statuses, drain-restart resume, 4004 supersede); WB3–WB8 not started ([docs/38](docs/38-windows-native-broadcaster.md)) |
