...
| R13 | [Advanced broadcaster settings](#r13--advanced-broadcaster-settings) | ✅ implemented 2026-07-15 (L1–L5); automated gates green, **manual browser verify passed 2026-08-03** ([docs/18](docs/18-advanced-broadcaster-settings.md)) |
| R14 | [Native Linux broadcaster](#r14--native-linux-broadcaster) | ✅ V0–V7 implemented 2026-07-15, automated gates green; **manual verify on the gaming PC done 2026-07-19** (hardware encode/portal/GUI — not CI-reachable); V8 (direct Vulkan Video) still gated on V2's on-hardware result, not started; **V9 (build version in the window) implemented 2026-08-01** ([docs/19](docs/19-linux-native-broadcaster.md)) |
...
| R18 | [Live viewer count](#r18--live-viewer-count) | ✅ Y1–Y6 implemented 2026-07-18, automated gates green; cluster viewer-count check (origin `viewersGlobal` == Σ per-pod real viewers, edges excluded) **automated in the `e2e-cluster` CI job 2026-07-19**; single-pod browser manual verify **passed 2026-08-03** (count matches across two viewers and decrements on leave) ([docs/23](docs/23-live-viewer-count.md)) |
...
| R23 | [Terms & conditions / usage terms](#r23--terms--conditions--usage-terms) | ✅ implemented 2026-07-24 (TC1–TC5); automated gates green (gawk-app + gawk-broadcast + helm), **manual browser verify passed 2026-08-03** ([docs/29](docs/29-terms-and-conditions.md)) |
| R24 | [Broadcaster capture & audio guidance](#r24--broadcaster-capture--audio-guidance) | ✅ designed + CG1–CG5 implemented 2026-07-24 (browser-aware share/audio tips + reactive notes; frontend-only, zero server/wire/pipeline change); automated gates green, **manual browser verify passed 2026-08-03** ([docs/30](docs/30-broadcaster-capture-audio-guidance.md)) |
...
| R30 | [Connection interleaving for live-edge delivery](#r30--connection-interleaving-for-live-edge-delivery) | 🔶 designed + **ST2–ST6 implemented 2026-07-29** (owner instruction moved implementation ahead of ST1); gates green in all four modules incl. a race-clean Go burst-threshold proof (control 8.3 % loss, 13/60 eighteen-chunk frames complete; striped ×3: 60/60 at 0.0 %, zero mismapped); §13.1 UI browser verify (ST5's manual half) **passed 2026-08-03** on desktop Chrome against the fleet; **ST1 paired on-hardware runs + ST7 acceptance/loadgen still owner-pending**; **ST8–ST10 leg-lifecycle correction implemented 2026-08-13** (orphaned-leg reap via ?owner= token + 20 s liveness lease, closing the 2026-08-03 BUGS.md leak) ([docs/35](docs/35-connection-interleaving.md) §12, §13, §14) |
| R31 | [Telemetry UI v2: a purpose-built diagnosis SPA](#r31--telemetry-ui-v2-a-purpose-built-diagnosis-spa) | ✅ designed + **TH1–TH11 implemented 2026-07-30**; gates green (Go race + `-tags duckdb`, oxlint / 49 UI tests / build, and the no-external-fetch test over the built bundle); both §8 open questions resolved by the owner — build-tagged cgo DuckDB, SSE kept with the poll as fallback; **on-hardware pass passed 2026-08-03** (driven against real accumulated session data over the internal Ingress) ([docs/36](docs/36-telemetry-ui-history.md) §9); read-surface only — zero wire/relay/viewer/broadcaster change |
...
| R54 | [Broadcasting over Wi-Fi](#r54--broadcasting-over-wi-fi) | 🔧 designed 2026-09-23 (owner decisions OD1–OD6 taken 2026-09-24), not started (WU0–WU6; WU4 deferred) — leg-A loss from a Mac on Wi-Fi (AWDL) freezes viewers; deltas ride one 150 ms-deadline reliable stream per GOP from the Rust desktop broadcaster (R19's carrier, reversed) behind a new `CapUplinkCarriers` relay capability, always on when supported, with a warned Advanced → Legacy escape hatch; QoS marking shipped; silent by default, a plain status line on measured harm; the AirDrop-pausing **Improve** helper decided after WU2 ([docs/56](docs/56-wifi-uplink.md)) |
...
docs/35 §12), **ST1/ST7 on-hardware halves owner-pending**. **ST8–ST10
implemented 2026-08-13** (docs/35 §14): the leg-lifecycle correction —
orphaned legs are reaped via a viewer-minted `?owner=` token plus a 20 s
liveness lease, closing the 2026-08-03 BUGS.md leak.
...
- **Served by `gawk-admin`**, unauthenticated like `/auth/config`, with
  the deployment's base URL substituted; **an embedded Redoc page** behind
  the normal login (Swagger UI in the first cut; swapped 2026-09-17 on how
  the page reads, losing "try it out" and, with it, the only reason a
  third-party bundle was handed an access token). **2026-09-21 (OA5)**: the
  page gains an in-house **Console** drawer — pick an operation, fill its
  parameters and the pre-filled example body, send as the signed-in
  operator through the session, read the response, copy the `curl` line
  with a `$TOKEN` placeholder. First-party code beside Redoc, which stays
  token-free; the request path is built relative from the document, so
  the token cannot leave the origin.
...
**Status**: ✅ shipped 2026-09-16 — chunks OA1–OA4 in
[docs/49](docs/49-admin-openapi.md), which records what deviated from the
design. `GET /api/v1/events` gained the `?type=` filter D3 (d) assumed and
the handler had never had; the list envelopes became declared Go types, which
is what makes the example round-trip a real check. R49 depends on this.
...
**Status**: designed 2026-09-18 (owner decisions OD1–OD10); **MB0 (the
rename) implemented 2026-09-22**, landing alone as D2 requires; **MB1
(workspace split, shared `crates/ui`, the `macos-latest` CI job, the macOS
shell) implemented and owner-verified 2026-09-23**; **MB2 (ScreenCaptureKit
capture through the system picker) implemented 2026-09-23**, its manual
pass owner-pending; **MB3 (VideoToolbox encode, the first real broadcast)
implemented 2026-09-23** on a shared shell core, its trial and
encode-to-relay criteria verified on an M1; **MB4 (ScreenCaptureKit audio on
the same stream) implemented 2026-09-23**, G1/G2/V-4 owner-pending; **MB5
(the macOS shell: Settings… ⌘,, notifications, the Share card) implemented
2026-09-23**; **MB6** telemetry in, its update notice/install **blocked on
R45/R47 (not started)**; **MB7 (signing, notarization, the macOS release
asset and manifest, site card, docs) implemented 2026-09-23**, the macOS
distribution switching on once the owner adds the five Apple secrets
(docs/54 §11); MB8 is the owner's on-hardware pass. Chunks in
[docs/54](docs/54-macos-native-broadcaster.md).
