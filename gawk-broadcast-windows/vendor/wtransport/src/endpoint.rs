        ...
        O: IntoConnectOptions,
    {
            ...
            .await
        {
            ...
            Err(ProtoWriteError::Stopped) => {
                // GAWK PATCH: no response ever arrived — status unknown.
                return Err(ConnectingError::SessionRejected(0));
            }
        ...
        if session_response.code().is_successful() {
            ...
            // GAWK PATCH: surface the rejection status instead of dropping it.
            return Err(ConnectingError::SessionRejected(
                session_response.code().into_inner(),
            ));
        }
