...
impl Driver {
    ...
    /// GAWK PATCH (see GAWK-PATCH.md): exposes the driver's terminal error
    /// so `Connection::closed()` can prefer the session-level close (the
    /// CLOSE_WEBTRANSPORT_SESSION capsule) over the QUIC-level bookkeeping
    /// close that follows it.
    pub(crate) async fn terminal_error(&self) -> Option<DriverError> {
        self.driver_result.result().await
    }
