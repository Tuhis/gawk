...
impl FrameKind {
    ...
    const fn parse(id: VarInt) -> Option<Self> {
        match id {
            ...
            // GAWK PATCH (see GAWK-PATCH.md): every other frame type is
            // treated like a grease frame and skipped — RFC 9114 §9 says
            // unknown frame types MUST be ignored, and quic-go sends GOAWAY
            // (0x07) on the control stream during shutdown, which stock
            // wtransport turned into a violation-close.
            id => Some(FrameKind::Exercise(id)),
        }
    }
