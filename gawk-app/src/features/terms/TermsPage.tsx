...
// The terms surface, reachable from every surface and gated behind nothing.
// Renders the bundled default unless the operator has set config.termsUrl,
// in which case that document is fetched on open, sanitized, and rendered
// instead, with the bundled default shown while the fetch is in flight and on
// any failure so the page is never blank. The fetch happens here (on route
// open), never at app boot.
    ...
    <div className={styles.root}>
      ...
      <GlassPanel className={styles.card}>
        ...
        <article className={styles.doc}>
          {override != null ? (
            // Sanitized above (allowlist, DOMParser-inert): the app's single
