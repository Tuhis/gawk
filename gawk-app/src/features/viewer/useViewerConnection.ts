// The viewer connection, decoupled from the cinematic UI. It runs the
// pipeline in a Web Worker (decoding + OffscreenCanvas rendering off the main
// thread) when the environment supports it, and falls back to the
...
// The MSE presentation surface for the screen. `probe` is null until the
// capability verdict is known — on the worker path that is the first codec
// event (the mime needs the negotiated codec); the main-thread pipeline
// fallback is verdict-false immediately, tier-3-only by design (the muxer
// lives in the worker, and iPhone runs the worker path). Segments do not
...
export interface PresentationState {
  ...
  // The Opus-in-MP4 verdict for this stream's audio lane. Null until audio is
  // observed (a video-only broadcast never probes); false-supported keeps the
  // native player video-only and the inline AudioWorklet audible.
  audioProbe: MseAudioProbe | null;
  arm: () => void;
  setSegmentSink: (cb: ((seg: MuxSegmentEvent) => void) | null) => void;
}

// The audio surface for the screen. `present` is false until audio is
// actually observed in the stream — every piece of audio UI hangs off it, so
// a video-only broadcast renders no audio UI at all.
...
export interface AudioState {
  ...
  resume: () => void;
  // Silence the inline AudioWorklet sink WITHOUT
...
// What a room tile needs that the single viewer does not. Read through refs —
// none of these re-run the session effect.
...
export interface ViewerConnectionState {
  ...
  // `error` message is console-only detail ("handshake failed" means nothing
  // to a viewer).
  errorKind: ViewerErrorKind | null;
  errorFatal: boolean;
  // 'moderated' when close code 4006 ended the broadcast. Only meaningful
  // while `status` is 'ended'.
  endReason: ViewerEndReason;
  retryNote: string | null;
  // The id this viewer reports under, once a hello has enabled collection —
  ...
  // nothing (from here indistinguishable from a relay without telemetry).
...
export function useViewerConnection(
  ...
  // The playout mode. Applied to whichever context the pipeline runs in
  // (worker command / main-thread module), live.
  playoutMode: PlayoutMode = 'off',
  // Frame interpolation (only effective on a WebGL2 worker sink in adaptive
  // mode; harmless elsewhere).
  interpolation = false,
  // Request the encoded-frame mux fork. True only on gated (element-
  // fullscreen-less) devices; false leaves the mux out of the worker
  // protocol entirely.
  presentationMux = false,
  // The delivery mode. Changing it re-runs the session effect — a deliberate
  // teardown + reconnect with (or without) ?delivery=reliable; the wider
  // reorder/playout profile is applied to the pipeline's context before the
  // session starts.
  deliveryMode = 'live' as ViewerDeliveryMode,
  // An opt-DOWN from the fleet parity default. Undefined
  ...
  // reconnect.
  parityLevel?: 0 | 1,
  // The stripe mode. Deliberately NOT in the session
  ...
  // Why the session ended, so the end card can say
  ...
  // The MSE capability verdict, computed on the main thread from the
  ...
  // Probed once, from the first stats tick that reports an audio
  ...
  // Where the screen's MsePresenter receives segments. A ref — segments
  // arrive up to once per frame and must never drive renders.
  const segmentSinkRef = useRef<((seg: MuxSegmentEvent) => void) | null>(null);

  // The audio sink lives here — one per hook instance, created lazily on the
  // first audio chunk so a video-only stream never constructs an AudioContext.
  ...
  // Video-master drift trim: alignment is fixed at playback start, so this is
  // the only lever left. Lives here because it
  ...
  // An override, not a preference — the sink is muted while either
  ...
  const ensureSink = useCallback((): AudioSink | null => {
    ...
    // resilient 500 ms, and live edge needs neither — a truthy check on the
    // mode would hand live edge the resilient profile.
    const profile = (): AudioBufferProfile =>
      audioProfileForDeliveryMode(deliveryModeRef.current);
    const sink = new AudioSink(
      {
        // The ~4 Hz playhead report reaches whichever context runs the
        // pipeline — into the worker as a command, or into this context's
        // module state on the fallback path. The sink stamps the pair itself:
        // with getOutputTimestamp() the moment a sample is at the listener is
        // part of the measurement, not "whenever this callback ran".
  ...
  // volume control would silently do nothing while the context stays blocked.
  ...
  // Telemetry rides the SAME event funnel both paths
  ...
  // The room key rides every batch of a tile session, whichever of the
  ...
  // The collector's own id, mirrored into render state so the overlay can
  ...
  // stream just died, and the dashboard keeps ended sessions in its own
  // group. A reconnect replaces it, because that is a new relay session
  ...
  const applyEvent = useCallback((ev: ViewerWorkerEvent) => {
    switch (ev.type) {
      ...
      case 'reconnecting':
        ...
        // show calmer copy than the generic attempt counter.
        setRetryNote(
      ...
      case 'codec':
        setCodec(ev.codec);
        // The MSE capability verdict needs the negotiated codec — this
        ...
        break;
      case 'stats': {
        // The audio jitter-buffer target rides
        ...
        if (sink) {
          ...
          // Video-master: hand the sink the
          ...
          setAudioNeedsGesture(sink.needsGesture);
          // The jitter-buffer counters live in the main-thread sink, so merge
          // them into the worker-assembled stats here — the one place with
          // both in hand — so the overlay and Copy diagnostics see buffer
          // depth / underruns / drops.
        ...
        // The audio lane's codec only becomes
      ...
      case 'ended':
        ...
        break;
      // Gated-device-only segment stream (the worker emits it only when
      ...
      case 'muxSegment':
        ...
        break;
      // Decoded PCM from the worker pipeline (the main-thread path
      ...
      case 'audioReset':
        handleAudioReset();
        break;
      // This session's telemetry identity. Adopting it is what starts
      ...
      case 'telemetryHello':
        ...
        break;
      // The relay-advertised ingest URL. Races the
  ...
  // The main-thread pipeline cannot host the worker muxer — tier 3 by design,
  // and the verdict is known without waiting for a codec event.
  useEffect(() => {
  ...
  // Session start/stop per broadcast id — and per delivery-mode flip, which
  // is a deliberate reconnect with the delivery negotiation in the URL. The
  // mode command goes first: worker messages process in order, so the
  // profile is live before the session starts.
  useEffect(() => {
    if (!useWorker) return;
    resetState();
  ...
  // The playout mode, applied on mount and on every toggle.
  ...
  useEffect(() => {
    ...
    let active = true;
    resetState();
    // The profile lives in this (main-thread) context here; set it
    ...
    const session = new ViewerSession(
      ...
      },
      {
        ...
        // The main-thread pipeline decodes audio in place and feeds the
  ...
  // Arm the worker muxer (screen calls this at `watching` on gated
