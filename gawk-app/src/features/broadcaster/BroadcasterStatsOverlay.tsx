...
interface Props {
  ...
  // Whether this browser can do audio at all. When false, the Audio State row
  // reads the honest "Not supported here" instead of the raw audioState
  // ('no-track' → the misleading "No audio shared").
...
// The broadcaster stats overlay, the counterpart of the viewer's — the encode funnel (capture → post-gate → encoded → sent) plus
  ...
  const sections: StatsSection[] = [
    {
      ...
      rows: [
        ...
        // Runtime truth: what configure() actually landed on, not what the
        // probe matrix predicted.
        ['Encode mode', encoderInfo?.acceleration ?? '—'],
        // The probe-resolved auto targets ('—' on explicit selections).
    ...
    },
    {
      title: 'Delivery',
      rows: [
        // The relay's live audience push (fleet-global in cluster mode).
    ...
    // The Audio section — present only when audio was requested, so a
    // video-only broadcast's overlay is unchanged.
      ...
      ? [
          {
            ...
            rows: [
              [
                'State',
                // A browser that can't do audio reads the honest "Not
                // supported here" whatever the raw state (Firefox lands on
                // 'no-track'/'unavailable', which would otherwise read "No
                // audio shared" — as if a checkbox would fix it).
                !audioSupported
                  ? OVERLAY_UNSUPPORTED
              ...
              // The encoder's own delay. It costs no lip sync as long as it
              // stays out of the audio timestamps — this row is what says
              // whether it did. Re-anchors are beside it because each one
              // steps the audio timeline.
              [
                'Encode lag',
