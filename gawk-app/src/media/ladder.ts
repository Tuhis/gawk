// The broadcast ladder: what the broadcaster can choose to send, and the
...
// "auto" is a selection, not a rung — the picker's resolution axis.
// Everything downstream (computeTargetSize, the preprocessor) stays concrete;
// the pipeline resolves 'auto' to the current auto-ladder rung. Auto first —
// it is the default.
...
// The framerate axis has the same shape. 'auto' resolves at probe time
// (framerate-first, resolveAutoFps below) and never runtime-steps — auto
// stepping is resolution-only.
...
// (1920x1080@60 ≈ 6.2 Mbps), scaled by sqrt(fps/60) so low-framerate streams
// keep enough headroom for crisp keyframes, clamped to [0.5, 10] Mbps.
const BITS_PER_PIXEL_AT_60 = 0.05;
const MIN_BITRATE = 500_000;
const MAX_BITRATE = 10_000_000;

// The per-source effective ladder auto mode walks:
...
// Structural mirror of the probe matrix's lookup — declared here (not
// imported) because probe.ts imports this module; the extra SupportEntry
...
// 'auto' fps resolves framerate-first — 60 when any rung probes hardware at
// 60, else the conservative 30. Never 'native': a 144 Hz monitor would
// multiply the fan-out for no viewer-visible benefit.
...
// When nothing probes hardware (Firefox; software-only mode) the auto
// ceiling is 1080p — a sane software starting point; auto stepping handles
// the rest.
...
// Capture-constraint width cap for a sticky rung: null for native — the
// broad grant, no cap beyond the original request.
...
// The advanced bitrate override is absolute — it replaces the ladder math
// until reset — but clamped to a wider band than the ladder's: the 1 Gbps
// homelab uplink allows experiments past the 10 Mbps ladder cap (15 viewers ×
// 50 Mbps stays within egress), while the floor matches the ladder's.
