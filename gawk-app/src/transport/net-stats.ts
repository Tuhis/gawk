// WebTransport connection health sampling. Each client measures its own leg
// to the relay — RTT, packet loss, send-rate ceiling — which is what lets a
// problem be attributed to broadcaster-uplink vs viewer-downlink without any
// cross-reporting channel.
//
// `WebTransport.getStats()` is spec'd but shipped by NO browser: Chromium
// removed its pre-spec implementation (the spec-conformant rewrite is tracked
// at https://chromestatus.com/feature/5194440034746368) and Firefox never had
// it. Every field is nullable and the sampler never throws: unsupported simply
// yields null and the overlays render "—". The mapping below matches the
// spec's WebTransportConnectionStats, so it lights up automatically when a
// browser ships it.
