// The viewer's incoming-datagram buffer.
//
...
// burst of ~9-11 datagrams (its chunks, then its two parity symbols, which the
// broadcaster deliberately sends last), 28 times a second. If the queue is
// shallower than a burst, the datagrams are evicted before the read loop is
// ever scheduled, and no reader can win that race.
//
// Losing the head of each burst is also the one loss shape a per-frame k=2
// code cannot repair — three erasures in one frame — so parity reads as
// "ineffective" while working exactly as designed (on Firefox: ~10 % of delta
// chunks gone against ~0.05 % of parity, on one connection).
//
...
// `incomingHighWaterMark` the original. Firefox ships only the old one, and
// Chromium is removing it, so neither name alone covers the fleet. Spec name
// first; they are aliases, so setting one is setting both.
...
//
// Measured, not assumed: on Firefox, a paired A/B against a live broadcast —
// two subscribers, same seconds, hwm 1 vs 8192, reader stalled to stress the
// queue — moves delivery by less than its own A/A noise floor.
// `e2e/firefox-datagram-buffer.mjs` re-runs it when Firefox ships the real
// attribute.
...
export interface DatagramBufferStats {
  ...
  // diagnostic number here: Firefox reports 1 — one datagram, against a
  // delta frame's burst of ~11.
