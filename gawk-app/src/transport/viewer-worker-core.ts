// Host-agnostic core of the worker-offloaded viewer. `viewer.worker.ts` is a
// thin `onmessage` shell around this; the core owns the viewer pipeline *and*
// the reconnect state machine (it reuses ViewerSession unchanged, so
...
export type ViewerWorkerCommand =
  // `presentationMux` is sent ONLY on gated (element-fullscreen-less)
  // devices. It routes the reorder buffer's released frames through the host
  // frame tap so the (arm-created) fMP4 muxer can feed the iPhone
  ...
  | { type: 'stop' }
  // Set the playout mode in the worker's context (the reorder buffer and
  // pipeline read it live, so this works mid-session and across reconnects).
  | { type: 'playout'; mode: PlayoutMode }
  // The experimental frame-interpolation toggle, same crossing.
  | { type: 'interpolation'; enabled: boolean }
  // The delivery mode for this worker's context — wider reorder/playout
  // profile. Sent before 'start' (the delivery negotiation itself rides
  // connectOpts.deliveryMode into the subscribe URL); a mode change is a
  // deliberate reconnect, not a live flip.
  | { type: 'resilient'; mode: ViewerDeliveryMode }
  // The stripe mode for this worker's context. Unlike 'resilient' this IS a
  // live flip — engagement is in-band (leg dials + the 0x10 level protocol),
  // so no reconnect is needed in either direction.
  | { type: 'stripeMode'; mode: StripeMode }
  // Start muxing — create the Fmp4Muxer at the worker-shell level (it
  // survives pipeline attempts/reconnects) and begin posting segments.
  // Idempotent; a no-op when init carried no mux flag. `audio` is the main
  // thread's negotiated encapsulation: 'opus' muxes the audio lane verbatim
  // (what Chrome takes), 'aac' transcodes the decoded PCM because iOS refuses
  // Opus in MP4. Absent keeps the presentation video-only.
  | { type: 'arm'; audio?: AudioMuxCodec }
  // The audio sink's ~4 Hz playhead report, travelling the reverse direction
  // of the stats flow. The AudioContext is main-thread-only, so this is how
  // the worker's pipeline gets an audio clock. atEpochMs is absolute
  // (timeOrigin + now) because the two contexts have different timeOrigins.
  // `heardUs` is the sample **at the listener**, not the worklet's write
  // position — the sink converts before sending, since it owns the only API
  // that can.
...
export type ViewerWorkerEvent =
  ...
  // `reason` distinguishes a moderator kill (close code 4006) from any other
  // clean ending, so the screen can say which happened.
  | { type: 'ended'; reason: ViewerEndReason }
  // Gated devices only, after 'arm': one fMP4 segment from the worker
  ...
  | MuxSegmentEvent
  // Decoded planar PCM for the main-thread AudioWorklet sink — the first
  // deliberate decoded-media crossing (an AudioContext cannot exist in a
  // dedicated worker). The channel buffers are transferred, so nothing is
  // structured-cloned.
  | {
      type: 'audioChunk';
      timestampUs: number;
  ...
  // Restart/reconnect — the sink must flush and re-anchor before the new
  // timeline's packets arrive.
  | { type: 'audioReset' }
  // The session's telemetry identity, forwarded to the main thread where
  // collection lives. Its own message rather than a field on 'stats' because
  // the token is a bearer credential and ViewerStats is what Copy diagnostics
  // serializes for pasting into a chat.
  | { type: 'telemetryHello'; hello: TelemetryHelloMessage }
  | { type: 'telemetryEndpoint'; url: string };

// The muxer's output events (see ViewerWorkerEvent above). `track` routes the
// segment to its own SourceBuffer on the main thread — video and audio are
// separate MSE streams.
...
// What the pipeline forks to the shell's muxer (defined beside the
...
// pipeline otherwise (Firefox worker WebCodecs is the main risk).
...
export interface WorkerHost {
  // The optional transfer list carries decoded audio buffers across
  // without a structured clone.
  post(event: ViewerWorkerEvent, transfer?: Transferable[]): void;
  renderSink: RenderSink;
  // How each pipeline attempt gets its transport. The shell supplies a
  // nested-transport-worker factory where nested workers exist; omitted, the
  // pipeline runs its transport in-process (this worker).
  transportFactory?: ViewerTransportFactory;
  // The encoded-frame fork. Present only when init carried `presentationMux`
  // (gated devices) — the pipeline then hands every reorder-released frame
  // here, upstream of the decoder, and the shell's muxer (idle until 'arm')
  // turns them into fMP4. Host-level so it survives pipeline
  // attempts/reconnects.
  frameTap?: (frame: ReleasedFrame) => void;
  // The encoded-audio fork, present under the same gate as frameTap.
  audioTap?: (ev: AudioTapEvent) => void;
  // The DECODED-audio fork (iOS path), for the AAC transcode iOS needs because
  // it refuses Opus in MP4. Fired before the decoded buffers are transferred to
  // the main thread.
...
export class ViewerWorkerCore {
  ...
  start(params: { serverUrl: string; broadcastId: string; connectOpts: ConnectOptions }): void {
    ...
    const cbs: ViewerSessionCallbacks = {
      ...
      // Decoded PCM crosses to the main thread, channel buffers transferred.
      onAudioChunk: (chunk) => {
        if (!current()) return;
        // iOS AAC path: transcode BEFORE the post — the channel buffers
      ...
      // The session's telemetry identity, out to the main thread where
      ...
      // The encoded-frame fork for the shell's muxer. Generation-guarded
      ...
      ...(this.host.frameTap
        ...
        : {}),
      // Same generation guard — a superseded session's audio must
