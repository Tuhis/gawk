...
## Stripe legs outlive the primary that owns them, holding subscriber slots indefinitely

- **Found**: 2026-08-03, reading the live fleet while diagnosing an unrelated
  Safari join failure on broadcast `DDP4H7`. Not reproduced deliberately — the
  state was simply there in `/statusz`.
- **Impact**: orphaned leg sessions hold real subscriber slots against
  `maxSubscribers` (100) and `maxTotalSubscribers` (400) for as long as the
  broadcast lives, and inflate `subscribers` / `gawk_broadcast_stripe_legs`.
  With R30 striping default-on in live-edge mode, every affected viewer leaks
  up to `wire.MaxStripeLegs` sessions. The R18 viewer count is *not* affected —
  legs deliberately never count there — so the surface that would make this
  visible to an operator is the one place it is invisible.
- **Observed state**, one pod, ~65 min after the triggering eviction:
  ```
  subscribers: 4    stripeLegs: 4    viewersGlobal: 0
  keyframeDrops: { openFailed: 10 }
  ```
  The relay log shows the sequence: four legs dial at 19:33:22 (`stripe_n: 4`),
  then at 19:34:27 the **primary** is evicted — `evicting unreachable
  subscriber: keyframe stream opens failing persistently`,
  `consecutive_open_failures: 10` (the R10 `KeyframeOpenFailEvictThreshold`
  path) — and its session ends. No leg session ever ends. `viewersGlobal: 0`
  with `subscribers: 4` is the signature: nobody is watching, four sessions
  remain.
- **Cause — structural, not a slipped case.** A leg is an independent
  subscriber created by `Registry.SubscribeStripeLeg` (`internal/hub/stripe.go`)
  with no back-reference to its primary; the registry has no notion that they
  belong together. docs/35 §5.6 assigns teardown to the **viewer**: *"Primary
  death is exactly today's session death: ViewerSession reconnects, legs are
  torn down with the pipeline (they are owned by the transport, which is
  per-attempt)."* That holds when the viewer notices. It does not hold when the
  relay kills the primary out from under a viewer that does not react — and a
  leg cannot age out on its own, because legs keep receiving delta datagrams,
  so the QUIC idle timeout never fires. Decision 4's `StripeStateTTL` does not
  cover this either: it self-heals the *suppression* state on a primary, and
  here the primary is gone.
- **Not yet established**: whether the viewer failed to tear down (tab
  backgrounded or already navigated away), or whether the eviction's close
  never surfaced to the stripe controller as leg-teardown at all. These want
  different fixes, and one live capture with the viewer still open would
  separate them.
- **Fix would start**: relay-side, because the relay is the party that created
  the orphan. Give the registry the primary→leg association it currently lacks
  (the legs are already dialed with the same broadcast and are distinguishable
  by `stripeLeg`), and reap a primary's legs when its session ends for any
  reason — eviction, close, or GC. That is the only fix that holds regardless
...
- **Related**: the eviction that triggered this is itself the keyframe-stream
  ...
  write. Which browser that session was is not recoverable from the logs.
