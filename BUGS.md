...
## Native broadcaster GUI burns 20–30 % CPU while idle

- **Found**: 2026-07-27, reported from the gaming PC — `gawk-broadcast-gui` sits
  at 20–30 % CPU from launch, before anything has been shared. Root-caused the
  same day by reading the widget source and confirming the mechanism in an
  isolated Gio harness (below); not yet re-measured on the box, and no fix is
  written.
- **Impact**: a constant CPU tax for a window that is doing nothing, on the one
  machine that is also running the game gawk exists to stream. It is
  **idle-only** — the spin stops when the broadcast goes live (see below), which
  is why the R14 on-hardware pass never saw it: that pass was watching the
  encode path.
- **Cause (confirmed in isolation)**: `component.TextField` (gioui.org/x
  v0.10.1) requests an immediate redraw on **every frame** whenever the field
  holds text. Its update rule sets `state = activated` whenever
  `Editor.Len() > 0`, and the branch `if in.state == activated || hasContents &&
  !in.focused` then calls `anim.Start(gtx.Now, Forward, 0)` unconditionally —
  the `in.focused` latch that looks like it prevents a repeat is short-circuited
  by the first term. A zero-duration `Progress` starts and finishes inside one
  frame (`Start` runs `Update`, which jumps progress to 1.0; the `Update` call a
  few lines later sees `Finished()` and `Stop()`s it), so the next frame starts
  it again — and `if in.anim.Started() { gtx.Execute(op.InvalidateCmd{}) }` runs
  in the gap between the two, every single time. An `InvalidateCmd` with no `At`
  means "redraw as soon as possible", so the window free-runs at the
  compositor's rate, re-shaping text and repainting the whole panel each pass.
- **Why from launch, and why only while idle**: `newUI` pre-fills the settings
  fields from the saved config (`u.relay.SetText(cfg.RelayURL)`, plus `appURL`,
  `secret`, `bitrate` — `cmd/gawk-broadcast-gui/main.go`), so `hasContents` is
  already true when the window first paints; a genuinely fresh config would not
  trip it. And Decision 9 lays the settings panel out through `gtx.Disabled()`
  once a broadcast starts — a disabled context carries a zero `input.Source`,
  which silently swallows the `InvalidateCmd` — so the spin ends exactly when
  the stream begins. The 20–30 % is therefore paid entirely in the state where
  nothing is happening.
- **Measured** (throwaway harness: one `component.TextField` laid out for 20
  frames against a real `input.Router`, counting frames whose `WakeupTime()` is
  already due): pre-filled **20/20**, empty **0/20**, pre-filled through
  `gtx.Disabled()` **0/20**.
- **Confirmation on the box**: move `~/.config/gawk/broadcast.json` aside
  (XDG-honoured path) and launch — CPU should sit near idle, then jump the
  moment a character is typed into any settings field. If it stays high with
  every field empty, there is a second redraw source and the above is only part
  of the story; the hand-rolled `dropdown` and `material.List` are the next
  suspects, though neither drives a gio-x animation today.
- **Fix would start**: locally, since the defect is upstream. In increasing
  order of work: (a) wrap or vendor the widget's update rule so the animation
  starts on a state *transition* rather than on every frame it spends
  `activated`; (b) drop `component.TextField` for a plain `material.Editor` plus
  the label/border this UI already hand-draws elsewhere — a floating-label
  animation buys nothing on a four-field settings panel and is the only thing
  gio-x is imported for; (c) fix it upstream and bump, the right long-term
  answer but not the one that unblocks the box. Whichever lands, the regression
  guard should be a frame-level assertion in the shape of the harness above
  ("an idle window requests no immediate redraws"), never a CPU measurement —
  CPU is machine-specific and would never fail in CI.
