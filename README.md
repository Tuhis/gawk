...
| Path | What |
|------|------|
| `ROADMAP.md` | High-level roadmap for post-v0.5 work (R1–R28), with ordering rationale and per-item scope sketches |
...
28. 🔧 Native broadcaster audio (R25): the Linux broadcaster publishes **system audio** alongside the screen, on by default — R15 built the feature and only the *browser* ever got a producer, so this is a producer change and nothing else: zero relay, wire, viewer and `gawk-app` changes. Capture is **outside the portal** (ScreenCast carries no audio) from the default output's monitor via PipeWire, through a probed source cascade whose lead candidate *follows* a headphone↔speaker switch instead of erroring; Opus is encoded by the same GStreamer child (48 kHz stereo, 20 ms, DTX and in-band FEC off — every property load-bearing for the "one packet per datagram" wire design) and **muxed into the existing MPEG-TS**, so both media share one PTS timeline and one `ptsAnchor` maps them with one affine function — relative A/V skew is zero by construction, where a second pipe would have quietly bought a constant lip-sync bias. Audio is **subordinate**: no usable source publishes video and says so, a live failure naming an audio element drops audio and retries the same encoder rung, and a whole-cascade failure re-runs once without audio so a GPU problem is never diagnosed as a sound card — `docs/28` (NA1 spike on real hardware + NA2–NA7 implemented 2026-07-27; automated gates green in both Go modules, and a new tier-1 e2e pass has a real headless Chrome decoding relayed Opus beside a still-video-only broadcast; the on-hardware A/V run (NA8) is pending, sequenced after R15's re-verification because `avSkewMs` is the instrument both need)
29. 🔧 Advanced diagnostics & telemetry (R28): a new **optional, default-off** [`gawk-telemetry/`](gawk-telemetry/) module — the fourth — that adds **no measurement** and is instead a pipe, a store and a query surface over the ~80-field `ViewerStats`, `BroadcastStats`, `engine.Stats` and `RegistryStats` that already exist. It closes the gap that made per-viewer questions unanswerable: `/statusz` named a subscriber by a random key and **nothing ever told the client its own key**, so the two sides could not be joined. New wire type 0x0D `TypeTelemetryHello` (35 B) hands each session a stateless-verifiable token on a reliable uni stream; `subscriberDetails` gains `sessionId`. Collection is out-of-band HTTPS on a **same-origin** path (telemetry must outlive the transport it reports on), the **relay is scraped and never pushes** (no queue in the process carrying every broadcast's hot path), storage is hive-partitioned NDJSON — 14 days raw, **permanent rollups**, retention a directory delete — and validation is **strict envelope, tolerant payload** because version skew is permanent, not transient. The payoff is `diagnose()`: docs/13's bottleneck playbook as 15 rules returning **ranked verdicts with provenance-tagged evidence, never raw series** (765 bytes for a 4-hour session), plus a **live operator dashboard** where severity sorts the list, ended broadcasts sit recessed below the live ones, and a viewer that stopped reporting reads *stale* or *unknown* — never green. Zero PII: no IPs, no raw user agents, no cross-session identity, never media. The viewer's stats overlay opens with the session id the relay gave it (the dashboard's own 8-character prefix; the full 24 in the tooltip and in Copy diagnostics) — the sessionId names a session, the token authorizes writing to one, so only the first is ever shown — `docs/33` (TM1–TM8 implemented 2026-07-26; automated gates green in all four modules; TM9 Grafana dropped by scope decision so R9 M8 stays open; manual verification pending)
...
- **A control loop's metric must measure where the user is, and stop reading
  ...
  now reads null instead. ([docs/20](docs/20-system-audio.md) field finding 13)
- **A metric that means two things means nothing.** `avSkewMs` was read in the
  ...
  behind, that reading is true. ([docs/20](docs/20-system-audio.md) field
  finding 12)
- **Don't smooth a measurement you were handed exactly.** The same metric's
  ...
  of skew out of an assumption. ([docs/20](docs/20-system-audio.md) field
  finding 12)
- **Stamp both media at the same pipeline stage.** Video is stamped at capture
  ...
  sync is wrong. ([docs/20](docs/20-system-audio.md) field finding 13)
