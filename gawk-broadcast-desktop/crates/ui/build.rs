//! Compiles the shared window and stamps the build revision. Both moved here
//! from the Windows shell's build script when the macOS shell arrived (docs/54
//! D1, D11). The stamp has to live in this crate because `cargo::rustc-env`
//! only reaches the package whose build script emits it, and `version` reads
//! it here.
...
/// Stamps the commit as `GAWK_BUILD_REV`, for the version badge in the
/// window header (see src/version.rs).
///
