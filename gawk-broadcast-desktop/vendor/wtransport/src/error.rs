...
pub enum ConnectingError {
    ...
    /// Request rejected.
    ///
    /// GAWK PATCH (see GAWK-PATCH.md): carries the HTTP `:status` of the
    /// rejected CONNECT response — 0 when the request stream was stopped
    /// before any response arrived. Upstream parses the status and discards
    /// it; gawk's resume supervisor and start-error messages need it
    /// (docs/38 D2 gate 2a).
    #[error("server rejected WebTransport session request (status {0})")]
    SessionRejected(u16),
