...
impl Connection {
    ...
    pub async fn closed(&self) -> ConnectionError {
        let quic_error: ConnectionError = self.quic_connection.closed().await.into();
        // GAWK PATCH (see GAWK-PATCH.md): when the peer ended the SESSION
        // with a CLOSE_WEBTRANSPORT_SESSION capsule, the driver records it
        // and then locally closes the QUIC connection — so the quinn-level
        // cause is `LocallyClosed` and the application close code would be
        // lost. Prefer the driver's session-level close; it is the one that
        // carries the code (docs/38 D2 gate 2b). The await is bounded: the
        // worker sets its result right after closing, and a worker that
        // died without setting resolves `None`.
        if let Some(crate::driver::DriverError::ApplicationClosed(close)) =
            self.driver.terminal_error().await
        {
            return ConnectionError::ApplicationClosed(close);
        }
        quic_error
    }
